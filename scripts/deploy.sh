...
# ========= Config =========
SERVICE_NAME=${SERVICE_NAME:-slack-nano-banana-bot}

REGION=${GOOGLE_LOCATION:-us-central1}

AR_LOCATION=${AR_LOCATION:-$REGION}

AR_REPO=${AR_REPO:-slack-nano-banana-apps}
...
SERVICE_URL=$(gcloud run deploy "${SERVICE_NAME}" \
  ...
  --no-cpu-throttling  \
  --project "${PROJECT_ID}" \
  --set-env-vars "SLACK_BOT_TOKEN=${SLACK_BOT_TOKEN},SLACK_SIGNING_SECRET=${SLACK_SIGNING_SECRET},GOOGLE_PROJECT=${PROJECT_ID},GOOGLE_LOCATION=${REGION},ALLOWED_SLACK_WORKSPACE=${ALLOWED_SLACK_WORKSPACE:-},REACTION_PROCESSING=${REACTION_PROCESSING:-},REACTION_COMPLETED=${REACTION_COMPLETED:-}" \
